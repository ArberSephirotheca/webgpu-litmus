...
@compute @workgroup_size(workgroupXSize) fn main(
  ...
  if (shuffled_workgroup < stress_params.testing_workgroups) {
    ...
    let global_base = shuffled_workgroup * u32(workgroupXSize);
    ...
    let x_0 = (global_base + id_0) * stress_params.mem_stride * 2u;
    let y_0 = (global_base + y_id_0) * stress_params.mem_stride * 2u + stress_params.location_offset;
    let y_1 = (global_base + y_id_1) * stress_params.mem_stride * 2u + stress_params.location_offset;
    let x_1 = (global_base + id_1) * stress_params.mem_stride * 2u;
